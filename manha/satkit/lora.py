...
class LoRa:
    ...
    def __init__(
        ...
        self._beacon_interval = 10000  # 10s default (currently unused)
        ...
        # Callbacks
        self._callbacks = {CALLBACK_TELEMETRY_REQUEST: None, CALLBACK_COMMAND: None}
    ...
    async def send_telemetry_and_listen(
        self,
        data: dict,
        target_addr: int = None,
        max_size: int = 200,
        listen_timeout: int = 5000,
    ) -> tuple:
        """
    ...
    async def send_telemetry(
        self, data: dict, target_addr: int = None, max_size: int = 200
    ) -> bool:
        """
    ...
    async def _send_multipart_json(
        self, json_str: str, target_addr: int, max_size: int
    ) -> bool:
        ...
        try:
            ...
            for part_num in range(1, total_parts + 1):
                ...
                packet = Packet(
                    target_addr, self.device_id, packet_json.encode("utf-8")
                )
    ...
    async def _send_packet_with_ack(
        self, packet: Packet, expected_ack_part: int, timeout_ms: int = 5000
    ) -> bool:
